...
class DashboardlyDataLoader { // type of an object
  ...
  getHashForUser(user_email) {
    return (echo md5( strtolower( trim(user_email))));
  }
  ...
  createTokenFromCredentials(email, password) {
    ...
    let sessionToken;
    let user;
    return this.query( // communicates with the table users.
      knex
      .select('id', 'password')
      .from('users')
      .where('email', email)
...
<<<<<<< HEAD
  ...
  updateBookmark(bookmarkId, bookmarkData) { // I added board Id
    // TODO: this is up to you to implement :)
    // const errors = validate.boardUpdate(boardData);
    // if (errors) {
    //   return Promise.reject({ errors: errors });
